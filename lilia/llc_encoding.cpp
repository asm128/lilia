...
static constexpr const char								base64Symbols[]													= "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
			...
			::llc::error_t								llc::base64Encode												(const ::llc::array_view<const ubyte_t> & inputBytes, ::llc::array_pod<char_t> & out_base64)	{
	...
	for(uint32_t iInputByte = 0, inputByteCount = inputBytes.size(); iInputByte < inputByteCount; iInputByte += 3) {
		...
		for(uint32_t iSingleIn = 0; iSingleIn < 3; ++iSingleIn) { 
			::llc::bit_view<uint8_t>									inputBits														= {&inputTriplet[iSingleIn], 8};
			::llc::reverse(inputBits);
		}
		const ::llc::bit_view<uint8_t>								inputBits														= inputTriplet;
		uint8_t														outputQuad			[4]											= {};
		uint32_t													iBitIn															= 0;
		for(uint32_t iSingleOut = 0; iSingleOut < ::llc::size(outputQuad); ++iSingleOut) {
			::llc::bit_view<uint8_t>									outputBits														= {&outputQuad[iSingleOut], 6};
			for(uint32_t iBitOut = 0; iBitOut < 6; ++iBitOut) 
				outputBits[iBitOut]										= (bool)inputBits[iBitIn++];
			::llc::reverse(outputBits);
			throw_if(outputQuad[iSingleOut] >= ::llc::size(base64Symbols) - 1, ::std::exception(""), "Out of range.");
	...
	return 0;
			...
			::llc::error_t								llc::base64Decode												(const ::llc::array_view<const char_t> & in_base64, ::llc::array_pod<ubyte_t> & outputBytes)	{
	...
	llc_necall(outputBytes.resize(outputBytes.size() + packsNeeded * 3), "Out of memory? outputBinary.size() : %u. packsNeeded : %u.", iOutputByte, packsNeeded);
	for(uint32_t iInput64 = 0, symbolCount = in_base64.size(); iInput64 < symbolCount; iInput64 += 4) {
		...
		for(uint32_t iSingleIn = 0; iSingleIn < 4; ++iSingleIn) {
			::llc::bit_view<uint8_t>									inputBits														= {&inputQuad[iSingleIn], 6};
			::llc::reverse(inputBits);
		}
		const ::llc::bit_view<uint8_t>								inputBits														= inputQuad;
		uint8_t														outputTriplet		[3]											= {};
		uint32_t													iBitIn															= 0;
		for(uint32_t iSingleOut = 0; iSingleOut < 3; ++iSingleOut) {
			::llc::bit_view<ubyte_t>									outputBits														= {&outputTriplet[iSingleOut], 8};
