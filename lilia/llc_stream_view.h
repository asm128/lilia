...
namespace llc
{
	...
	struct stream_view : public array_view<_tElement> {
		...
		template<typename _tPOD>					inline	::llc::error_t					read_pod							(_tPOD* read, uint32_t nCount)	{ const int32_t						sizeToCopy = sizeof(_tPOD) * nCount ;				memcpy(read, &Data[CursorPosition], sizeToCopy);	CursorPosition += sizeToCopy;		return sizeToCopy; }
		template<typename _tPOD, size_t _sCount>	inline	::llc::error_t					read_pod							(_tPOD (&read)[_sCount])		{ static constexpr const int32_t	sizeToCopy = (int32_t)(sizeof(_tPOD) * _sCount);	memcpy(read, &Data[CursorPosition], sizeToCopy);	CursorPosition += sizeToCopy;		return sizeToCopy; }
		template<typename _tOBJ>							::llc::error_t					read_obj							(_tOBJ* read, uint32_t nCount)	{ 
