...
#pragma pack(push, 1)	// You can read about pragma pack() here: https://www.google.com/search?q=pragma+pack
	...
	struct SCoord2 {
		...
		inline									TCoord2&				Normalize				()																						{ const _tBase sqLen = LengthSquared(); return (sqLen) ? *this /= ::llc::sqrt_safe(sqLen) : *this;						}
		...
		constexpr								double					AngleWith				(const TCoord2& other)												const				{ double lengthsProduct = Length() * other.Length(); return lengthsProduct ? ::acos(Dot(other) / lengthsProduct) : 0;	}
			...
			const ::llc::SPairSinCos											pairSinCos				= ::llc::getSinCos(theta);
	...
	struct SCoord3 {
		...
		inline									TCoord3&				Normalize				()																						{ const _tBase sqLen = LengthSquared(); return (sqLen) ? *this /= ::llc::sqrt_safe(sqLen) : *this;						}
	...
	struct SQuaternion {
		...
		inline				TQuat&			Normalize				()																	noexcept	{ _tBase sqLen = LengthSquared(); if(sqLen ) return *this /= ::llc::sqrt_safe(sqLen); Identity(); return *this;		}
		...
		inline				void			SetRotation				(const TQuat& q1, const TQuat& q2)									noexcept	{ return *this = q1 * q2 * ~q1;																							}
			...
			if(fDot < 1.00001f && fDot > 0.99999f) 
				return ::llc::interpolate_linear(p, q_, fTime); 
		...
		inline				TQuat&			MakeFromEulerTaitBryan	(const TCoord3& v)																{ return MakeFromEulerTaitBryan(v.x, v.y, v.z);																			}
			...
			const SPairSinCos						pairSinCosX				= ::llc::getSinCos(fPitch	);
			const SPairSinCos						pairSinCosY				= ::llc::getSinCos(fYaw	);
			const SPairSinCos						pairSinCosZ				= ::llc::getSinCos(fRoll	);
...
#define LLC_DEFAULT_OPERATOR_NE(_otherType, ...)	\
	...
	template<typename _tElement>	struct SRange			{ _tElement									Offset, Count	; LLC_DEFAULT_OPERATOR_NE(SRange		<_tElement>, Offset	== other.Offset	&& Count	== other.Count					); };
	template<typename _tElement>	struct SLine2D			{ ::llc::SCoord2<_tElement>					A, B			; LLC_DEFAULT_OPERATOR_NE(SLine2D		<_tElement>, A		== other.A		&& B		== other.B						); };
	template<typename _tElement>	struct STriangle2D		{ ::llc::SCoord2<_tElement>					A, B, C			; LLC_DEFAULT_OPERATOR_NE(STriangle2D	<_tElement>, A		== other.A		&& B		== other.B		&& C == other.C	); };
	template<typename _tElement>	struct SRectangle2D		{ ::llc::SCoord2<_tElement>					Offset, Size	; LLC_DEFAULT_OPERATOR_NE(SRectangle2D	<_tElement>, Offset	== other.Offset	&& Size		== other.Size					); };
	template<typename _tElement>	struct SCircle2D		{ double Radius; ::llc::SCoord2<_tElement>	Center			; LLC_DEFAULT_OPERATOR_NE(SCircle2D		<_tElement>, Center	== other.Center	&& Radius	== other.Radius					); };
	template<typename _tElement>	struct SSphere2D		{ double Radius; ::llc::SCoord2<_tElement>	Center			; LLC_DEFAULT_OPERATOR_NE(SSphere2D		<_tElement>, Center	== other.Center	&& Radius	== other.Radius					); };

	template<typename _tElement>	struct SLine3D			{ ::llc::SCoord3<_tElement>					A, B			; LLC_DEFAULT_OPERATOR_NE(SLine3D		<_tElement>, A		== other.A		&& B		== other.B						); };
	template<typename _tElement>	struct SRectangle3D		{ ::llc::SCoord3<_tElement>					Offset, Size	; LLC_DEFAULT_OPERATOR_NE(SRectangle3D	<_tElement>, Offset	== other.Offset	&& Size		== other.Size					); };
	template<typename _tElement>	struct SCircle3D		{ double Radius; ::llc::SCoord3<_tElement>	Center			; LLC_DEFAULT_OPERATOR_NE(SCircle3D		<_tElement>, Center	== other.Center	&& Radius	== other.Radius					); };
	...
	template<typename _tElement>
							STriangle3D<_tElement>&						translate								(::llc::STriangle3D<_tElement>& triangle, const ::llc::SCoord3<_tElement>& translation)									{
		triangle.A															+= translation;
		triangle.B															+= translation;
		triangle.C															+= translation;
		return triangle;
	...
	template<typename _tElement>
							STriangle3D<_tElement>&						scale								(::llc::STriangle3D<_tElement>& triangle, const ::llc::SCoord3<_tElement>& scale)									{
...
#pragma pack(pop)
	...
	template<typename _tElement>	static					double					sphereSize				(const SSphere2D<_tElement> &sphere)										noexcept	{ return 1.3333333333333333 * ::llc::math_pi * sphere.Radius * sphere.Radius * sphere.Radius; }
...
#pragma pack(pop)
	...
	template <typename _tCoord>
				::llc::SRectangle2D<_tCoord>&									realignRectangle		
					(	const ::llc::SCoord2<uint32_t>			& targetSize
					,	const ::llc::SRectangle2D<_tCoord>		& rectangleToRealign
					,	::llc::SRectangle2D<_tCoord>			& rectangleRealigned
					,	ALIGN							align
					)																																					noexcept	{
	...
	//------------------------------------------------------------------------------------------------------------
	template <typename _tCoord>
					::llc::SCoord2<_tCoord>&										realignCoord		
					(	const ::llc::SCoord2<uint32_t>		& targetSize
					,	const ::llc::SCoord2<_tCoord>		& coordToRealign
					,	::llc::SCoord2<_tCoord>				& coordRealigned
					,	::llc::ALIGN						align
					)																																					noexcept	{
	...
	static constexpr	bool														in_range		(const ::llc::SCoord2<_tValue>& valueToTest, const ::llc::SCoord2<_tValue>& rangeStart, const ::llc::SCoord2<_tValue>& rangeStop)	noexcept	{ 
	...
	template<typename _tValue>	
	static constexpr	bool														in_range		(const ::llc::SCoord2<_tValue>& pointToTest, const ::llc::SRectangle2D<_tValue>& area)	noexcept	{ 
		return	::llc::in_range(pointToTest.x, area.Offset.x, area.Offset.x + area.Size.x) 
			&&	::llc::in_range(pointToTest.y, area.Offset.y, area.Offset.y + area.Size.y)
			;
	}

	template<typename _tCoord>
	static inline		double														determinant		(const ::llc::SLine2D<_tCoord>& line)													noexcept	{ return ::llc::determinant((double)line.A.x, (double)line.A.y, (double)line.B.x, (double)line.B.y); }
}

#endif // LLC_COORD_H_928374982364923322