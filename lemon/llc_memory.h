...
namespace llc
{
	...
	static inline void*																			llc_malloc					(size_t size)													{ return _aligned_malloc(size, LLC_MALLOC_ALIGN);	}
	static inline void																			llc_free					(void* ptr)														{ _aligned_free(ptr);										}
#elif defined(LLC_LINUX) || defined(LLC_ANDROID)
	static inline void*																			llc_malloc					(size_t size)													{ return ::memalign(LLC_MALLOC_ALIGN, size);			}
	static inline void																			llc_free					(void* ptr)														{ ::free(ptr);												}
#endif

	template<typename _typePtr>
	static inline void																			safe_llc_free				(_typePtr &p)													{ 
		_typePtr																						_pepe						= p; 
		p																							= 0; 
		_aligned_free(_pepe);
	}

	struct auto_llc_free : public ::llc::auto_handler<void*, nullptr>					{ using TWrapper::auto_handler; inline ~auto_llc_free() { close(); } inline void close() { safe_llc_free(Handle); } };
